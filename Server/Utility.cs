...
namespace Server
{
	...
	{
		public static readonly int UPDATERANGE = 18;
...
#pragma warning restore 618
		...
		{
			return (p1.m_X >= (p2.m_X - UPDATERANGE))
				&& (p1.m_X <= (p2.m_X + UPDATERANGE))
				&& (p1.m_Y >= (p2.m_Y - UPDATERANGE))
				&& (p1.m_Y <= (p2.m_Y + UPDATERANGE));
		}
		...
		{
			return (p1.X >= (p2.X - UPDATERANGE))
				&& (p1.X <= (p2.X + UPDATERANGE))
				&& (p1.Y >= (p2.Y - UPDATERANGE))
				&& (p1.Y <= (p2.Y + UPDATERANGE));
		}
		...
		#region SA
		public static List<int> RandomizeList(List<int> tor)
		{
			int[] ar = new int[tor.Count];
			tor.CopyTo(ar);
			List<int> ricorsivecopy = new List<int>(ar);
			List<int> toreturn = new List<int>();
			return DoRandomizeList(ricorsivecopy, toreturn);
		}
		private static List<int> DoRandomizeList(List<int> tora, List<int> outg)
		{
			if (tora.Count > 0)
			{
				int key = RandomMinMax(0, tora.Count - 1);
				outg.Add(tora[key]);
				tora.Remove(tora[key]);
				return DoRandomizeList(tora, outg);
			}
			else
				return outg;
		}
		/// <summary>
		/// Returns a randomized list of "count" items
		/// </summary>
		/// <param name="count"></param>
		/// <returns></returns>
		public static List<int> RandomizedList(int count)
		{
